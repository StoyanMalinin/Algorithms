...
int main()
{
    ...
    for(int i = 0;i<m;i++)
    {
        ...
        queries.push_back(new AddQuery(u, v));
    }
    ...
    while(Q--)
    {
        ...
        if(t==1) queries.push_back(new AskQuery(u, v));
        else if(t==2) queries.push_back(new AddQuery(u, v));
        else if(t==3) queries.push_back(new RemQuery(u, v));
    }

    vector <bool> ans;
    evaluate(n, queries, ans);
    for(const Query *q: queries) delete q;
