...
int main()
{
    ...
    if(k>=2)
    {
        dp[2][n-1][0] = min(dp[0][n-1][0], -dp[0][n-1][0]);
        dp[2][n-1][1] = min(dp[0][n-1][1], -dp[0][n-1][1]);
        for(int i = n - 2;i>=0;i--)
        {
            dp[2][i][0] = min(-dp[1][i][1], a[i] + dp[2][i+1][0]);
            dp[2][i][1] = max(-dp[1][i][0], a[i] + dp[2][i+1][1]);
        }


        ans = min(ans, dp[2][0][0]);
    }
